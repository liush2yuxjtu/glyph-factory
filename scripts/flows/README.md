...
`tests/flows-sync.test.mjs` 断言 **`flows.json` 与当场重算的逐字节一致**，并要求每一幕的表里
都有推钟动词与微事件。生成物只断言「存在」是守不住的——手写过一次，重排之后表里留着
「viral-word 送 750 读者」和「map-city 是第二章的出口」，两条都不是真的了，而所有断言全绿。
