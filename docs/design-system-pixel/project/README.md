...
- `accent`（灯绿）只给**主动词**（`is-major`）、库存文字那一格和「已完成」。
- `lamp`（灯金）只给**微事件**（`is-event`）、世界尺度角标和进度条上的「当前」段。绿与金上的字一律 `on-accent`。
...
- 玩家面只说世界语言：「还差 意义（12 / 30）」「花掉 库存字 30」。不出现 A##、ACT 编号解释、机制名。幕名与幕号只在评审面出现。
