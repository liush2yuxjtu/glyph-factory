...
所有 PR 都执行验证，不用 path filter 跳过稳定检查名。
顺序：快速契约验证 → 同一个 dist artifact 的 Chromium/WebKit 测试 → `Player merge gate` → 显式部署。
任何必需任务失败、取消或跳过，汇总门禁都会失败。浏览器测试不配置自动重试。
截图、Playwright trace 和测试日志保留为 GitHub 开发者制品，不进入 Vercel 的玩家包。

仓库管理员应在 main 的保护规则中将 `Player merge gate` 设置为必需状态检查。
工作流文件本身不能自动修改 GitHub 管理员级保护设置。
Vercel Git 自动生成的预览与 GitHub 工作流显式部署是两个入口；预览 READY 不代表浏览器验证通过。
只有同一提交的完整门禁成功，才可标记“已验证”，不能沿用旧提交的绿色状态。
